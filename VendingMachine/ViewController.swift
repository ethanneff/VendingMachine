...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
  ...
  func setupCollectionViewCells() {
    // dynamic sizing for the collection view items
  ...
  @IBAction func purchase() {
    if let currentSelection = currentSelection {
      do {
        ...
        showAlert("Insufficent Funds", message: "Additional $\(amount) needed to complete the transaction")
      } catch let error {
        fatalError("\(error)")
      }
    } else {
      // FIXME: needs a selection
    }
